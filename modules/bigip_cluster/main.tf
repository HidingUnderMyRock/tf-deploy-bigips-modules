...
resource "aws_network_interface" "mgmt-enis" {
    ...
    tags = {
        Name                    = "${var.awsNamePrefix}-${var.awsClusterName}-${count.index+1}-${var.awsSubnetSuffix}-eth0"
    }
}
...
resource "aws_network_interface" "external-enis" {
    ...
    tags = {
        Name                    = "${var.awsNamePrefix}-${var.awsClusterName}-${count.index+1}-${var.awsSubnetSuffix}-eth1"
    }
    private_ips_count           = var.awsSecondaryIpCount
    
    # Write address info to file upon instance creation
    provisioner "local-exec" {
        command = "echo External: ${self.private_ip} >> ${var.awsNamePrefix}-${var.awsClusterName}-${count.index+1}-${var.awsSubnetSuffix}.info"
    }
}
...
resource "aws_network_interface" "internal-enis" {
    ...
    tags = {
        Name                    = "${var.awsNamePrefix}-${var.awsClusterName}-${count.index+1}-${var.awsSubnetSuffix}-eth2"
    }
    
    # Write address info to file upon instance creation
    provisioner "local-exec" {
        command = "echo Internal: ${self.private_ip} >> ${var.awsNamePrefix}-${var.awsClusterName}-${count.index+1}-${var.awsSubnetSuffix}.info"
    }
}
...
resource "aws_instance" "f5_bigip" {
    ...
    tags = {
        Name                    = "${var.awsNamePrefix}-${var.awsClusterName}-${count.index+1}-${var.awsSubnetSuffix}"
    }
    user_data                   = file("cloud-init.yaml")
    
    # Write address info to file upon instance creation
    provisioner "local-exec" {
        command = "echo Mgmt-int: ${self.private_ip} >> ${var.awsNamePrefix}-${var.awsClusterName}-${count.index+1}-${var.awsSubnetSuffix}.info"
    }
    provisioner "local-exec" {
        command = "echo Mgmt-ext: ${self.public_ip} >> ${var.awsNamePrefix}-${var.awsClusterName}-${count.index+1}-${var.awsSubnetSuffix}.info"
    }
    provisioner "local-exec" {
        command = "echo Public-DNS: ${self.public_dns} >> ${var.awsNamePrefix}-${var.awsClusterName}-${count.index+1}-${var.awsSubnetSuffix}.info"
    }
    
    # Delete address info file upon instance destruction - Windows syntax
    provisioner "local-exec" {
        when    = destroy
        command = "del ${var.awsNamePrefix}-${var.awsClusterName}-${count.index+1}-${var.awsSubnetSuffix}.info"
    }
}